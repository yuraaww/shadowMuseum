...
function initObjModel(){
    ...
    mtlLoader.setPath('../model/cargoship/');

    // cargoship 1
    mtlLoader.load('cargoship.mtl', function (materials) {
        materials.preload(); //材質預先載入
        var objLoader = new OBJLoader();
        objLoader.setCrossOrigin('Anonymous'); // 如果檔案來自IE，會有跨域問題 ，用來解決跨域問題
        objLoader.setMaterials(materials);
        objLoader.setPath('../model/cargoship/');
        objLoader.load(
            'cargoship.obj',
            function (obj) {
                //是否設置陰影
...
window.onload = function () {
    ...
    shipObj1_scale_x = 0.1;    
    shipObj1_scale_y = 0.1; 
    shipObj1_scale_z = 0.1; 
